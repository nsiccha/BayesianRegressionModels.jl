...
const WARMUP = parse(Int, get(ENV, "WEIGHTS_BENCH_WARMUP", "10000"))
const SAMPLES = parse(Int, get(ENV, "WEIGHTS_BENCH_SAMPLES", "21"))
const ITERATIONS = parse(
    Int, get(ENV, "WEIGHTS_BENCH_ITERATIONS", "20000"))
...
const replicates = [1 + mod(7 * row, 5) for row in 1:N]
const group_truth = [0.42 * sin(0.31 * index) for index in 1:GROUPS]
const mu_truth = [
    0.18 - 0.33 * x[row] + group_truth[group[row]] for row in 1:N
]
const y = [
    mu_truth[row] + 0.51 / sqrt(replicates[row]) * sin(0.53 * row)
    for row in 1:N
]
const data = (; x, group, replicates, y)
...
NP.@model function natural_weighted_varying_intercept_benchmark(
        x, group, replicates)
...
const natural = NP.condition(
    natural_weighted_varying_intercept_benchmark(x, group, replicates); y)
...
const pure_data = "{" * join((
    ...
    "\"weight\":" * array_json(replicates),
    "\"y\":" * array_json(y)), ",") * "}"
const pure_stan_path = joinpath(
    @__DIR__, "benchmark_native_ppl_weights.stan")
const pure_stan_compile_path = "/tmp/native_ppl_weights_pure.stan"
...
normalized_stan_density(density) = density - GROUPS * log_tau
...
println("context=", (;
    ...
    weight_range=extrema(replicates),
